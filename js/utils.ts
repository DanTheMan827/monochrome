//js/utils.js
import { qualityBadgeSettings, coverArtSizeSettings, trackDateSettings } from './storage.ts';
...
export const getTrackTitle = (track: Record<string, unknown>, { fallback = 'Unknown Title' } = {}): string => {
    if (!track?.title) return fallback;
    return track?.version ? `${track.title} (${track.version})` : track.title;
};

export const getTrackArtists = (track: Record<string, unknown> = {}, { fallback = 'Unknown Artist' } = {}): string => {
...
export const getTrackArtistsHTML = (track: Record<string, unknown> = {}, { fallback = 'Unknown Artist' } = {}): string => {
    if (track?.artists?.length) {
