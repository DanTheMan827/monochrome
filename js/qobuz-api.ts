...
export class QobuzAPI {
    ...
    async fetchWithRetry(endpoint, options: { signal?: AbortSignal } = {}) {
    ...
    // Search tracks
    async searchTracks(query, options: { offset?: number; limit?: number } = {}) {
        try {
    ...
    // Search albums
    async searchAlbums(query, options: { offset?: number; limit?: number } = {}) {
        try {
            const offset = options.offset || 0;
            const limit = options.limit || 10;
            const data = await this.fetchWithRetry(
    ...
    // Search artists
    async searchArtists(query, options: { offset?: number; limit?: number } = {}) {
        try {
            const offset = options.offset || 0;
            const limit = options.limit || 10;
            const data = await this.fetchWithRetry(
    ...
    async search(query, options: { offset?: number; limit?: number } = {}) {
        const offset = options.offset || 0;
        const limit = options.limit || 10;
