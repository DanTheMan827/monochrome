...
export class ListenBrainzScrobbler {
    ...
    _getMetadata(track: any): ListenBrainzTrackMetadata | null {
    ...
    async submitListen(listenType: string, track: any, timestamp: number | null = null) {
        if (!this.isEnabled()) return;

        const metadata = this._getMetadata(track);
        if (!metadata) return;

        const payload: ListenPayloadItem[] = [
            {
