...
export function loadFfmpeg(): Promise<{ coreURL: string; wasmURL: string }> {
    return (
        (loadFfmpeg as { promise?: Promise<{ coreURL: string; wasmURL: string }> }).promise ||
        ((loadFfmpeg as { promise?: Promise<{ coreURL: string; wasmURL: string }> }).promise = (async () => {
            const data = {
