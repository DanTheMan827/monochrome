...
export class LibreFmScrobbler {
    ...
    _getScrobbleArtist(track: TrackInfo | null): string {
    ...
    async generateSignature(params: Record<string, string>): Promise<string> {
        ...
        try {
            const { default: md5 } = await import('./md5.js') as { default: (s: string, key?: string, raw?: boolean) => string };
    ...
    async makeRequest(method: string, params: Record<string, string | number | undefined> = {}, requiresAuth: boolean = false): Promise<Record<string, any>> {
    ...
    async loveTrack(track: TrackInfo): Promise<void> {
        if (!this.isAuthenticated()) return;
