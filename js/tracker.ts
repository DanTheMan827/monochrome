...
interface TrackerArtist {
    name: string;
    url: string;
}

interface TrackerCardElement extends HTMLElement {
    _eraData?: TrackerEra;
    _artistName?: string;
    _sheetId?: string;
}

let artistsData: TrackerArtist[] = [];
...
function transformErasImages(eras: Record<string, TrackerEra>): Record<string, TrackerEra> {
    if (!eras) return eras;
...
async function fetchTrackerData(sheetId: string): Promise<TrackerData | null> {
    const endpoints = [
...
export async function renderTrackerArtistPage(sheetId, container) {
    ...
    // Set up header
    const imageEl = document.getElementById('tracker-artist-detail-image') as HTMLImageElement;
    ...
    eras.forEach((era) => {
        ...
        const card = projectsContainer.lastElementChild as TrackerCardElement;
        card._eraData = era;
    ...
    searchInput.addEventListener('input', (e) => {
        ...
        if (matches.length === 0) {
            ...
            // Add click handlers
            tracklist.querySelectorAll<HTMLElement>('.track-item').forEach((item) => {
...
export async function renderTrackerProjectPage(sheetId, projectName, container, _ui) {
    ...
    const imageEl = document.getElementById('album-detail-image') as HTMLImageElement;
    ...
    tracklistContainer.querySelectorAll<HTMLElement>('.track-item').forEach((item) => {
        const track = trackDataStore.get(item);
        if (!track || track.unavailable) return;
    ...
    if (moreAlbumsSection && moreAlbumsContainer) {
        ...
        if (otherEras.length > 0) {
            ...
            moreAlbumsContainer.querySelectorAll<HTMLElement>('.card').forEach((card) => {
...
export async function renderTrackerTrackPage(trackId, container, _ui) {
    ...
    // Add click handler
    const trackItem = tracklistContainer.querySelector<HTMLElement>('.track-item');
