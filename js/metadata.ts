...
import { fetchTagLib, addMetadataWithTagLib, getMetadataWithTagLib } from './taglib';
import { doTimed, doTimedAsync } from './doTimed';
import type { TagLibMetadata } from './taglib.types';
...
export function prefetchMetadataObjects(track, api, coverBlob = null) {
    ...
    const lyricsFetch = (managers as any)?.lyricsManager?.fetchLyrics?.(track.id, track)?.catch(console.error);
...
export async function addMetadataToAudio(audioBlob, track, api, _quality, prefetchPromises) {
    ...
    try {
        ...
        return doTimed(
            ...
            () =>
                new Blob([newAudioBuffer as BlobPart], {
                    type: audioBlob.type,
                })
        );
    } catch (err) {
...
export async function readTrackMetadata(file, siblings = []) {
    ...
    try {
        ...
        if (data) {
            ...
            metadata.artist = (data.artist || metadata.artist) as { name: string };
            ...
            if (data.cover) {
                const blob = new Blob([data.cover.data as BlobPart], { type: data.cover.type });
