...
export function initializeUIInteractions(player, api, ui) {
    ...
    if (libraryPage) {
        libraryPage.addEventListener('dragstart', (e) => {
            const playlistCard = (e.target as HTMLElement).closest('.card.user-playlist') as HTMLElement | null;
            if (playlistCard) {
        ...
        const handleDragOver = (e) => {
            const folderCard = (e.target as HTMLElement).closest('.card[data-folder-id]') as HTMLElement | null;
        ...
        const handleDrop = async (e) => {
            ...
            if (folderCard) {
                ...
                if (playlistId && folderId) {
                    const updatedFolder = await db.addPlaylistToFolder(folderId, playlistId) as any;
    ...
    const renderQueueControls = (container) => {
        ...
        if (addToPlaylistBtn) {
            addToPlaylistBtn.addEventListener('click', async () => {
                const playlists = await db.getPlaylists() as any[];
                ...
                modal.addEventListener('click', async (e) => {
                    if ((e.target as HTMLElement).classList.contains('modal-overlay') || (e.target as HTMLElement).classList.contains('cancel-btn')) {
                        closeModal();
                        return;
                    }

                    const option = (e.target as HTMLElement).closest('.modal-option') as HTMLElement | null;
                    if (option) {
                ...
                if (contextMenu) {
                    ...
                    if (track) {
                        ...
                        if (trackMixItem) {
                            ...
                            (trackMixItem as HTMLElement).style.display = hasMix ? 'block' : 'none';
                        }

                        positionMenu(contextMenu, e.clientX, e.clientY);

                        (contextMenu as any)._contextTrack = track;
                    }
                }
            });
    ...
    document.querySelectorAll('.search-tab').forEach((tab) => {
        tab.addEventListener('click', () => {
            ...
            // Track tab change
            trackSearchTabChange((tab as HTMLElement).dataset.tab);
            ...
            const contentId = `${prefix}${(tab as HTMLElement).dataset.tab}`;
    ...
    document.querySelectorAll('.settings-tab').forEach((tab) => {
        tab.addEventListener('click', () => {
            ...
            const contentId = `settings-tab-${(tab as HTMLElement).dataset.tab}`;
            document.getElementById(contentId)?.classList.add('active');

            // Save active tab
            import('./storage.js').then(({ settingsUiState }) => {
                settingsUiState.setActiveTab((tab as HTMLElement).dataset.tab);
            });
        });
    });
