...
function sortTracks(tracks: TrackData[], sortType: string): TrackData[] {
    ...
    switch (sortType) {
        case 'added-newest':
            return sorted.sort((a: TrackData, b: TrackData) => (b.addedAt || 0) - (a.addedAt || 0));
        case 'added-oldest':
            return sorted.sort((a: TrackData, b: TrackData) => (a.addedAt || 0) - (b.addedAt || 0));
        case 'title':
...
export class UIRenderer {
    ...
    createAlbumCardHTML(album: TrackAlbum): string {
        ...
        if (album.artist) {
            ...
            artistName = album.artists.map((a: TrackArtist) => a.name).join(', ');
        }
    ...
    renderListWithTracks(container: HTMLElement, tracks: TrackData[], showCover: boolean, append: boolean = false, useTrackNumber: boolean = false): void {
        ...
        const hasMultipleDiscs = tracks.some((t: TrackData) => (t.volumeNumber || t.discNumber || 1) > 1);
    ...
    setupHomeTabs(): void {
        ...
        tabs.forEach((tab) => {
            ...
            tab.addEventListener('click', () => {
                ...
                document.querySelectorAll<HTMLElement>('.home-view').forEach((v: HTMLElement) => {
    ...
    async renderExplorePage(): Promise<void> {
        ...
        try {
            ...
            if (GENRES.length > 0) {
                ...
                genresGrid.querySelectorAll<HTMLElement>('.genre-card').forEach((card: HTMLElement) => {
            ...
            if (data.sections && data.sections.length > 0) {
                data.sections.forEach((section: Record<string, unknown>) => {
                    if (section.items && (section.items as unknown[]).length > 0) {
                        ...
                        if (type) {
                            this.renderExploreSection(container, section.title as string, section.items as Record<string, unknown>[], type);
                        }
                    }
                });
            }
    ...
    renderExploreSection(container: HTMLElement, title: string, items: Record<string, unknown>[], type: string): void {
        ...
        if (type === 'track') {
            ...
            this.renderListWithTracks(list, items as unknown as TrackData[], true);
            ...
            items.forEach((item) => {
                ...
                if (type === 'album') selector = `[data-album-id="${(item as Record<string, unknown>).id}"]`;
                if (type === 'playlist') selector = `[data-playlist-id="${(item as Record<string, unknown>).uuid}"]`;

                if (selector) {
                    const el = grid.querySelector(selector);
                    if (el) {
                        trackDataStore.set(el as HTMLElement, item as unknown as TrackData);
                        if (type === 'album') this.updateLikeState(el as HTMLElement, 'album', (item as Record<string, unknown>).id as string);
                        if (type === 'playlist') this.updateLikeState(el as HTMLElement, 'playlist', (item as Record<string, unknown>).uuid as string);
                    }
                }
            });
    ...
    async renderGenrePage(genreId: string | undefined, genreName: string | undefined): Promise<void> {
        const container = document.getElementById('explore-grid');
        if (!container) return;
            ...
            if (data.sections && data.sections.length > 0) {
                data.sections.forEach((section: Record<string, unknown>) => {
                    if (section.items && (section.items as unknown[]).length > 0) {
                        ...
                        if (type) {
                            this.renderExploreSection(contentContainer, section.title as string, section.items as Record<string, unknown>[], type);
                        }
                    }
                });
            }
    ...
    async renderHomeEditorsPicks(forceRefresh: boolean = false, containerId: string = 'home-editors-picks'): Promise<void> {
        ...
        if (picksContainer) {
            ...
            try {
                ...
                const itemsToStore: { el: null; data: unknown; type: string }[] = [];
                ...
                if (cardsHTML.length > 0) {
                    ...
                    itemsToStore.forEach((item, _index) => {
                        ...
                        const id = (item.data as { id?: unknown })?.id;
    ...
    async renderAlbumPage(albumId: string | number, provider: string | null = null): Promise<void> {
        ...
        try {
            ...
            if (albumMenuBtn) {
                albumMenuBtn.dataset.id = String(album.id);
                trackDataStore.set(albumMenuBtn, album as unknown as TrackData);
            }
        ...
        try {
            ...
            if (playlistData) {
                ...
                recentActivityManager.addPlaylist({
                    ...
                    name: (playlistData!.name || playlistData!.title) as string | null | undefined,
                    title: (playlistData!.title || playlistData!.name) as string | null | undefined,
                    uuid: (playlistData!.uuid || playlistData!.id) as string | number | null | undefined,
                    cover: playlistData!.cover as string | null | undefined,
                    images: uniqueCovers,
    ...
    async renderArtistPage(artistId: string | number, provider: string | null = null): Promise<void> {
        ...
        try {
                                ...
                                unreleasedContainer.querySelectorAll('.card').forEach((card: Element) => {
                                    ...
                                    (card as HTMLElement).onclick = (e: MouseEvent) => {
                                        if ((e.target as HTMLElement).closest('.card-play-btn')) {
                                            ...
                                            const availableTracks = eraTracks.filter((t: TrackData) => !t.unavailable);
            ...
            recentActivityManager.addArtist(artist as unknown as { id: string | number });
    ...
    updatePlaylistHeaderActions(playlist: PlaylistData | Record<string, unknown>, isOwned: boolean, tracks: TrackData[], showShare: boolean = false, onSort: ((sortType: string) => void) | null = null, getCurrentSort: (() => string) | null = null): void {
        ...
        if (onSort) {
            ...
            sortBtn.onclick = (e: MouseEvent) => {
                ...
                const hasAddedDate = tracks.some((t: TrackData) => t.addedAt);
        ...
        if (showShare || (isOwned && (playlist as PlaylistData).isPublic)) {
