...
export class LastFMScrobbler {
    ...
    saveSession(sessionKey: string, username: string) {
    ...
    async generateSignature(params: Record<string, any>) {
        ...
        try {
            const { default: md5 } = await import('./md5.js') as { default: (str: string) => string };
    ...
    async makeRequest(method: string, params: Record<string, any> = {}, requiresAuth: boolean = false) {
    ...
    async authenticateWithCredentials(username: string, password: string) {
        try {
            const params = {
                username: username,
                password: password,
