...
function mapHeaders(rawHeaders: string[]): MappedHeaders {
...
export async function importToLibrary(csvResult, db, onProgress) {
    ...
    for (const [playlistName, playlistTracks] of Object.entries(csvResult.playlists) as [string, any[]][]) {
...
export async function parseXML(xmlText, api, onProgress) {
    ...
    let trackElements: HTMLCollectionOf<Element> = xmlDoc.getElementsByTagName('track');
