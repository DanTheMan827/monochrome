...
export async function showAddToPlaylistModal(track) {
    ...
    const renderModal = async () => {
        const playlists = (await db.getPlaylists(true)) as any[];
    ...
    const handleOptionClick = async (e) => {
        ...
        if (option.classList.contains('create-new-option')) {
            ...
            // Pass track
            (createModal as any)._pendingTracks = [track];
...
export async function handleTrackAction(
    ...
    if (action === 'start-radio' || action === 'start-infinite-radio') {
        ...
        if (type === 'track') {
            ...
            const playlist = (await db.getPlaylist(item.id)) as any;
    ...
    if (isCollection && collectionActions.includes(action)) {
        try {
            ...
            if (type === 'album') {
                ...
                let playlist: any = await db.getPlaylist(item.id);
                if (!playlist) {
                    try {
    ...
    if (action === 'add-to-queue') {
        ...
        const elementsToUpdate = [...document.querySelectorAll(selector)] as HTMLElement[];
        ...
        const handleOptionClick = async (e) => {
            ...
            if (option.classList.contains('create-new-option')) {
                ...
                // Pass track
                (createModal as any)._pendingTracks = [item];
        ...
        const storedHref = (contextMenu as any)?._contextHref;
    ...
    mainContent.addEventListener('click', async (e) => {
        ...
        if (
            ...
            if (isSearch) {
                ...
                const allTrackElements: HTMLElement[] = Array.from(parentList.querySelectorAll('.track-item'));
                const trackList = allTrackElements.map((el: HTMLElement) => trackDataStore.get(el)).filter(Boolean);
