    ...
    saveInstances(instances, type) {
        if (type) {
            try {
                ...
                const fullObj = stored ? JSON.parse(stored) : { api: [], streaming: [] };
...
export const fontSettings = {
    ...
    loadPresetFont(family, fallback = 'sans-serif') {
        const link = document.getElementById(this.FONT_LINK_ID);
        if (link) {
            link.remove();
        }

        const style = document.getElementById(this.FONT_FACE_ID);
        if (style) {
            style.remove();
        }
    ...
    loadAppleMusicFont() {
        ...
        const existingLink = document.getElementById(this.FONT_LINK_ID);
        if (existingLink) {
            existingLink.remove();
        }

        // Remove any existing @font-face styles
        const existingStyle = document.getElementById(this.FONT_FACE_ID);
        if (existingStyle) {
