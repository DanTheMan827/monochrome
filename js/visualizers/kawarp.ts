...
export class KawarpPreset {
    ...
    async lazyInit(
        ...
        try {
            ...
            const coverEl = document.querySelector('.now-playing-bar .cover') as HTMLImageElement | null;
    ...
    private _observeCoverArt(): void {
        ...
        this._coverObserver = new MutationObserver(() => {
            const el = document.querySelector('.now-playing-bar .cover') as HTMLImageElement | null;
    ...
    private _loadCover(url: string): void {
        ...
        this.kawarp!.loadImage(`${url}${sep}${CACHE_BUST_PARAM}`).catch((err) =>
