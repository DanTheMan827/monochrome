...
export class MusicDatabase {
    ...
    // Generic Helper
    async performTransaction<T = unknown>(storeName: string, mode: IDBTransactionMode, callback: (store: IDBObjectStore) => IDBRequest): Promise<T> {
    ...
    // History API
    async addToHistory(track: Record<string, any>): Promise<Record<string, any>> {
        ...
        return new Promise<Record<string, any>>((resolve, reject) => {
            ...
            transaction.onerror = (e: Event) => reject((e.target as IDBRequest).error);
        });
    }
    ...
    // Favorites API
    async toggleFavorite(type: string, item: Record<string, any>): Promise<boolean> {
    ...
    async isFavorite(type: string, id: string | number): Promise<boolean> {
        const plural = type === 'mix' ? 'mixes' : `${type}s`;
        const storeName = `favorites_${plural}`;
        try {
    ...
    async getFavorites(type: string): Promise<Record<string, any>[]> {
        const plural = type === 'mix' ? 'mixes' : `${type}s`;
        const storeName = `favorites_${plural}`;
        const db = await this.open();
        return new Promise<Record<string, any>[]>((resolve, reject) => {
    ...
    _minifyItem(type: string, item: Record<string, any>): Record<string, any> {
        if (!item) return item;
    ...
    _minifyPinnedItem(item: Record<string, any>, type: string): Omit<PinnedItemData, 'pinnedAt'> | null {
        if (!item) return null;
    ...
    async togglePinned(item: Record<string, any>, type: string): Promise<boolean | void> {
    ...
    async importData(data: Record<string, any>, clear: boolean = false): Promise<boolean> {
        const db = await this.open();

        const importStore = async (storeName: string, items: any): Promise<boolean> => {
            ...
            return new Promise<boolean>((resolve, reject) => {
                ...
                transaction.onerror = (event: Event) => {
                    console.error(`${storeName}: Transaction error:`, (event.target as IDBRequest).error);
    ...
    _updatePlaylistMetadata(playlist: UserPlaylistData): UserPlaylistData {
    ...
    _dispatchPlaylistSync(action: string, playlist: Record<string, any>): void {
    ...
    // User Playlists API
    async createPlaylist(name: string, tracks: Record<string, any>[] = [], cover: string = '', description: string = ''): Promise<UserPlaylistData> {
    ...
    async addTrackToPlaylist(playlistId: string, track: Record<string, any>): Promise<UserPlaylistData | void> {
        const playlist = await this.performTransaction<UserPlaylistData>('user_playlists', 'readonly', (store) => store.get(playlistId));
    ...
    async addTracksToPlaylist(playlistId: string, tracks: Record<string, any>[]): Promise<UserPlaylistData> {
        const playlist = await this.performTransaction<UserPlaylistData>('user_playlists', 'readonly', (store) => store.get(playlistId));
        if (!playlist) throw new Error('Playlist not found');
        playlist.tracks = playlist.tracks || [];
    ...
    async removeTrackFromPlaylist(playlistId: string, trackId: string | number, trackType: string | null = null): Promise<UserPlaylistData> {
        const playlist = await this.performTransaction<UserPlaylistData>('user_playlists', 'readonly', (store) => store.get(playlistId));
        if (!playlist) throw new Error('Playlist not found');
        playlist.tracks = playlist.tracks || [];
    ...
    async getPlaylist(playlistId: string): Promise<UserPlaylistData> {
        return await this.performTransaction<UserPlaylistData>('user_playlists', 'readonly', (store) => store.get(playlistId));
    }

    async updatePlaylist(playlist: UserPlaylistData): Promise<UserPlaylistData> {
        playlist.updatedAt = Date.now();
        this._updatePlaylistMetadata(playlist);
        await this.performTransaction('user_playlists', 'readwrite', (store) => store.put(playlist));
    ...
    async addPlaylistToFolder(folderId: string, playlistId: string): Promise<UserFolderData> {
    ...
    // User Folders API
    async createFolder(name: string, cover: string = ''): Promise<UserFolderData> {
        const id = crypto.randomUUID();
        const folder = {
            id: id,
            name: name,
    ...
    async getFolders(): Promise<Record<string, any>[]> {
        const db = await this.open();
        return new Promise<Record<string, any>[]>((resolve, reject) => {
    ...
    async getFolder(id: string): Promise<UserFolderData> {
        return await this.performTransaction<UserFolderData>('user_folders', 'readonly', (store) => store.get(id));
    }
    ...
    async getPinned(): Promise<PinnedItemData[]> {
        const storeName = 'pinned_items';
        const db = await this.open();
        return new Promise<PinnedItemData[]>((resolve, reject) => {
            const transaction = db.transaction(storeName, 'readonly');
            const store = transaction.objectStore(storeName);
            const request = store.getAll();
    ...
    async getPlaylists(includeTracks: boolean = false): Promise<Record<string, any>[]> {
        const db = await this.open();
        return new Promise<Record<string, any>[]>((resolve, reject) => {
    ...
    async updatePlaylistName(playlistId: string, newName: string): Promise<UserPlaylistData> {
        const playlist = await this.performTransaction<UserPlaylistData>('user_playlists', 'readonly', (store) => store.get(playlistId));
        if (!playlist) throw new Error('Playlist not found');
    ...
    async updatePlaylistDescription(playlistId: string, newDescription: string): Promise<UserPlaylistData> {
        const playlist = await this.performTransaction<UserPlaylistData>('user_playlists', 'readonly', (store) => store.get(playlistId));
        if (!playlist) throw new Error('Playlist not found');
    ...
    async updatePlaylistTracks(playlistId: string, tracks: Record<string, any>[]): Promise<Record<string, any>> {
        const db = await this.open();
        return new Promise<Record<string, any>>((resolve, reject) => {
