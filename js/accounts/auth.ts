...
export class AuthManager {
    ...
    async init(): Promise<void> {
        ...
        if (userId && secret && userId !== 'null' && secret !== 'null') {
            try {
                ...
                console.warn('OAuth session handoff failed:', getErrorMessage(error));
                window.history.replaceState({}, '', window.location.pathname);
            }
    ...
    async signInWithEmail(email: string, password: string): Promise<AppwriteUser | undefined> {
        try {
    ...
    async signUpWithEmail(email: string, password: string): Promise<AppwriteUser | undefined> {
        try {
    ...
    updateUI(user: AppwriteUser | null): void {
        ...
        if (window.__AUTH_GATE__) {
            ...
            if (customDbBtn) {
                ...
                if (pbFromEnv) {
                    const settingItem: HTMLElement | null = customDbBtn.closest('.setting-item') as HTMLElement | null;
